import asyncio # ⚡ 异步 I/O 库
import random # 🎲 随机数生成库
import time # ⏱️ 时间模块
from Memory.system_config import VIEWPORT # ⚙️ 导入视口配置

class MouseController:
    # =================================
    #  🎉 鼠标控制器 (无参数)
    #
    #  🎨 代码用途：
    #     作为 Angel 的“手”，负责执行具体的页面交互操作，如点击、移动、滑动等，并模拟人类的操作习惯。
    #     关键特性：会在页面中注入一个“虚拟光标”元素，确保在截图/视频流中能看到鼠标位置。
    #
    #  💡 易懂解释：
    #     Angel 的小手手！✋ 它可以帮你点点点，滑滑滑。而且它还会变魔术，在屏幕上变出一个小红点，
    #     这样你就知道它正指着哪里啦！
    #
    #  ⚠️ 警告：
    #     坐标计算依赖于 VIEWPORT 配置。虚拟光标通过 JS 注入，如果页面刷新，需要重新注入（已自动处理）。
    # =================================
    def __init__(self, page):
        # =================================
        #  🎉 初始化手部 (Playwright页面对象)
        #
        #  🎨 代码用途：
        #     绑定 Playwright 的 Page 对象，以便控制鼠标。
        #
        #  💡 易懂解释：
        #     把手伸进浏览器里！👋 准备好干活啦！
        # =================================
        self.page = page # 📄 绑定的页面实例
        self.cursor_id = "angel-ai-cursor" # 🆔 虚拟光标的 DOM ID
        self.last_action_time = 0 # ⏱️ 最后一次操作的时间戳

    def _mark_active(self):
        # =================================
        #  🎉 标记活跃状态 (无参数)
        #
        #  🎨 代码用途：
        #     更新最后操作时间戳。用于通知 WebSocket 服务当前处于“人机协作”模式，需要高频推流。
        #
        #  💡 易懂解释：
        #     Angel 刚刚动了一下！🕒 记下时间，告诉系统现在我很活跃，不要偷懒，画面要传快点！
        #
        #  ⚠️ 警告：
        #     仅更新时间戳，不触发任何实际操作。
        # =================================
        self.last_action_time = time.time()

    async def _ensure_cursor_visible(self):
        # =================================
        #  🎉 确保光标可见 (无参数)
        #
        #  🎨 代码用途：
        #     检查页面中是否存在虚拟光标元素，如果不存在（如页面刷新后），则重新注入 HTML 和 CSS。
        #     光标样式为一个带红点的半透明圆圈，层级最高 (z-index: 999999)。
        #
        #  💡 易懂解释：
        #     检查一下小红点还在不在？如果被网页“吃”掉了，就赶紧再变一个出来！✨
        #
        #  ⚠️ 警告：
        #     如果页面正在加载或已关闭，evaluate 可能会失败，已做异常捕获。
        # =================================
        if not self.page: return
        
        # 注入光标的 JS 代码
        js_code = f"""
        (id) => {{
            if (!document.getElementById(id)) {{
                const cursor = document.createElement('div');
                cursor.id = id;
                cursor.style.position = 'fixed';
                cursor.style.width = '20px';
                cursor.style.height = '20px';
                cursor.style.borderRadius = '50%';
                cursor.style.backgroundColor = 'rgba(255, 0, 0, 0.5)';
                cursor.style.border = '2px solid white';
                cursor.style.pointerEvents = 'none'; // 🚫 确保不阻挡点击
                cursor.style.zIndex = '999999';
                cursor.style.transition = 'top 0.1s, left 0.1s, transform 0.1s'; // 🌊 平滑动画
                cursor.style.transform = 'translate(-50%, -50%)'; // 🎯 居中定位
                cursor.style.boxShadow = '0 0 10px rgba(255,0,0,0.5)';
                document.body.appendChild(cursor);
            }}
        }}
        """
        try:
            await self.page.evaluate(js_code, self.cursor_id)
        except Exception:
            pass # 忽略页面已关闭等错误

    async def _update_cursor_visual(self, x, y, click_effect=False):
        # =================================
        #  🎉 更新光标视觉 (X坐标，Y坐标，是否点击特效)
        #
        #  🎨 代码用途：
        #     通过 JS 更新虚拟光标的位置。如果 click_effect 为 True，则触发点击动画（缩放）。
        #
        #  💡 易懂解释：
        #     小红点，快跟上！🏃‍♀️ 跑到这里来！如果是点击，就闪一下！✨
        #
        #  ⚠️ 警告：
        #     频繁调用 evaluate 可能会影响性能，建议仅在必要时更新。
        # =================================
        if not self.page: return
        
        scale = "scale(0.8)" if click_effect else "scale(1)"
        color = "rgba(255, 0, 0, 0.8)" if click_effect else "rgba(255, 0, 0, 0.5)"
        
        js_code = f"""
        (params) => {{
            const cursor = document.getElementById(params.id);
            if (cursor) {{
                cursor.style.left = params.x + 'px';
                cursor.style.top = params.y + 'px';
                cursor.style.transform = 'translate(-50%, -50%) {scale}';
                cursor.style.backgroundColor = '{color}';
            }}
        }}
        """
        try:
            await self.page.evaluate(js_code, {'id': self.cursor_id, 'x': x, 'y': y})
        except Exception:
            pass

    async def human_move(self, end_x, end_y):
        # =================================
        #  🎉 拟人化移动 (目标X坐标，目标Y坐标)
        #
        #  🎨 代码用途：
        #     将鼠标移动到指定坐标，并同步更新虚拟光标位置。
        #
        #  💡 易懂解释：
        #     慢慢移过去... 🚶‍♀️ 就像人手拿鼠标一样，有快有慢，不会“嗖”的一下就飞过去，这样才不会被发现是机器人在操作！
        # =================================
        """拟人化移动"""
        if not self.page: return
        self._mark_active() # ⏱️ 标记活跃
        
        await self._ensure_cursor_visible() # 👁️ 确保光标存在
        
        steps = random.randint(5, 10) # ⚡ 减少步数以提高响应速度，但保留平滑感
        
        # 执行 Playwright 真实移动
        await self.page.mouse.move(end_x, end_y, steps=steps) 
        
        # 同步更新视觉光标 (只在终点更新，避免通信过于频繁)
        await self._update_cursor_visual(end_x, end_y)

    async def click(self, x_ratio, y_ratio):
        # =================================
        #  🎉 拟人化点击 (X轴比例，Y轴比例)
        #
        #  🎨 代码用途：
        #     根据相对比例计算绝对坐标，先移动鼠标，模拟反应延迟，然后执行按下和抬起操作。
        #     同时触发视觉光标的点击动画。
        #
        #  💡 易懂解释：
        #     瞄准... 发射！👉 先看准位置移过去，稍微停顿一下（假装在思考），然后轻轻点下去，再松开。完美！
        #
        #  ⚠️ 警告：
        #     输入参数是 0.0-1.0 的比例值，而非绝对像素坐标。
        # =================================
        """拟人化点击"""
        if not self.page: return
        self._mark_active() # ⏱️ 标记活跃
        
        target_x = x_ratio * VIEWPORT['width'] # 📏 计算绝对 X 坐标
        target_y = y_ratio * VIEWPORT['height'] # 📏 计算绝对 Y 坐标
        
        # 1. 移动
        await self.human_move(target_x, target_y) # 🖱️ 移动到位
        
        # 2. 反应延迟
        await asyncio.sleep(random.uniform(0.05, 0.1)) # ⏳ 模拟人类反应时间
        
        # 3. 点击视觉特效
        await self._update_cursor_visual(target_x, target_y, click_effect=True)
        
        # 4. 真实点击
        await self.page.mouse.down() # 👇 按下鼠标
        await asyncio.sleep(random.uniform(0.05, 0.1)) # ⏳ 模拟按键时长
        await self.page.mouse.up() # ☝️ 抬起鼠标
        
        # 5. 恢复视觉特效
        await self._update_cursor_visual(target_x, target_y, click_effect=False)

    async def mouse_down(self, x_ratio, y_ratio):
        """按下鼠标"""
        if not self.page: return
        self._mark_active()
        
        target_x = x_ratio * VIEWPORT['width']
        target_y = y_ratio * VIEWPORT['height']
        
        await self._ensure_cursor_visible()
        await self.page.mouse.move(target_x, target_y) # 确保位置正确
        await self._update_cursor_visual(target_x, target_y, click_effect=True) # 视觉反馈
        await self.page.mouse.down()

    async def mouse_move(self, x_ratio, y_ratio):
        """移动鼠标"""
        if not self.page: return
        self._mark_active()
        
        target_x = x_ratio * VIEWPORT['width']
        target_y = y_ratio * VIEWPORT['height']
        
        await self._ensure_cursor_visible()
        # 移动时不带平滑步数，以保证实时性
        await self.page.mouse.move(target_x, target_y)
        # 保持点击特效（如果是拖拽中）- 这里简化为始终显示点击态可能不太好，
        # 但为了拖拽时的视觉连贯性，我们可以假设 move 通常发生在 down 之后。
        # 或者我们可以不传 click_effect，让它恢复默认，或者由调用者控制状态。
        # 简单起见，move 时我们不强制改变 click_effect，但为了视觉跟手，我们更新位置。
        # 如果是拖拽，通常希望保持“按下”的视觉状态。
        # 由于 mouse_move 是无状态的，我们无法知道当前是否 down。
        # 暂时不强制 click_effect=True，除非我们维护一个内部状态 self.is_pressed。
        await self._update_cursor_visual(target_x, target_y, click_effect=True) 

    async def mouse_up(self, x_ratio, y_ratio):
        """抬起鼠标"""
        if not self.page: return
        self._mark_active()
        
        target_x = x_ratio * VIEWPORT['width']
        target_y = y_ratio * VIEWPORT['height']
        
        await self._ensure_cursor_visible()
        await self.page.mouse.move(target_x, target_y)
        await self.page.mouse.up()
        await self._update_cursor_visual(target_x, target_y, click_effect=False) # 恢复视觉

    async def drag(self, start_x, start_y, end_x, end_y):
        """执行完整的拖拽操作 (绝对坐标)"""
        if not self.page: return
        self._mark_active()
        
        await self._ensure_cursor_visible()
        
        # 1. 移动到起点
        await self.page.mouse.move(start_x, start_y)
        await self._update_cursor_visual(start_x, start_y, click_effect=False)
        await asyncio.sleep(0.1)
        
        # 2. 按下
        await self.page.mouse.down()
        await self._update_cursor_visual(start_x, start_y, click_effect=True)
        await asyncio.sleep(0.1)
        
        # 3. 移动到终点 (带步数平滑)
        steps = 10
        await self.page.mouse.move(end_x, end_y, steps=steps)
        await self._update_cursor_visual(end_x, end_y, click_effect=True)
        await asyncio.sleep(0.1)
        
        # 4. 抬起
        await self.page.mouse.up()
        await self._update_cursor_visual(end_x, end_y, click_effect=False)

    async def scroll(self, delta_y):
        # =================================
        #  🎉 页面滚动 (Y轴滚动量)
        #
        #  🎨 代码用途：
        #     模拟鼠标滚轮滚动页面。
        #
        #  💡 易懂解释：
        #     滑轮滚一滚，页面走一走！🎡 往下看更多精彩内容！
        # =================================
        if not self.page: return
        self._mark_active() # ⏱️ 标记活跃
        await self.page.mouse.wheel(0, delta_y)

    async def jump_video(self, timestamp):
        # =================================
        #  🎉 视频跳转 (时间戳)
        #
        #  🎨 代码用途：
        #     控制页面中的视频播放器跳转到指定时间点。
        #
        #  💡 易懂解释：
        #     快进！⏩ 直接跳到精彩的地方！
        #
        #  ⚠️ 警告：
        #     当前为简化版实现，尚未注入具体的 JS 代码来控制 Video 标签。
        # =================================
        """控制视频跳转"""
        if not self.page: return False # 🚫 页面不存在，返回失败
        print(f"✋ Mouse: Jumping to {timestamp}s") # 📢 打印跳转日志
        # 这里需要注入 JS 来控制 video 标签
        # 简化版实现
        return True # ✅ 模拟跳转成功
