...
goto :eof
REM ==========================================
# 🚀 Web_compute_low 启动器 (静态文件服务器)
# 端口: 3000
# 环境: WSL2 + Rust
# ==========================================

$ErrorActionPreference = "SilentlyContinue"
$Host.UI.RawUI.WindowTitle = "Web_compute_low (Port 3000)"

# 错误日志路径
$errorLogPath = $env:ERROR_LOG_PATH
if (-not $errorLogPath) {
    $errorLogPath = ".\logs\Web_compute_low.log"
}
...
while ($true) {
    ...
    if ($restartCount -eq 0) {
        Write-Host " 🚀 Web_compute_low 启动中" -ForegroundColor Cyan
    } else {
        Write-Host " 🔄 Web_compute_low 重启中 (第 $restartCount 次)" -ForegroundColor Cyan
    }
    Write-Host " 📦 静态文件服务器 (Port 3000)" -ForegroundColor Cyan
    Write-Host "========================================`n" -ForegroundColor Cyan

    Write-Log "Web_compute_low 启动脚本开始执行"
    ...
    # 3. 检查项目目录
    if (-not (Test-Path "Web_compute_low\Cargo.toml")) {
        Write-Log "找不到 Web_compute_low 项目目录" $true
        Write-Host "❌ 错误: 找不到 Web_compute_low 项目" -ForegroundColor Red
        Read-Host "`n按回车键重试..."
        $restartCount++
        continue
    }
    ...
    Write-Host "`n🧹 清理端口 3000..." -ForegroundColor Cyan
    ...
    Write-Host "  ✅ 端口 3000 已清理" -ForegroundColor Green
    ...
    Write-Log "WSL 路径: $wslPath/Web_compute_low"

    try {
        Write-Host "🚀 启动服务..." -ForegroundColor Green
        
        $output = wsl bash -c "cd '$wslPath/Web_compute_low' && ~/.cargo/bin/cargo run --bin web_compute_low_server 2>&1"
