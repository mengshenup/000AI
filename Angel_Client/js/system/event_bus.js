class EventBus {
    ...
    // =================================
    constructor() {
        this.listeners = {}; // 📚 存储所有事件监听器的字典 { 'event_name': [callback1, ...] }
    }
    ...
    on(event, callback) {
        ...
        if (!this.listeners[event]) {
            this.listeners[event] = []; // 🆕 初始化该事件的监听列表
        }
        // 把回调函数加入数组
        this.listeners[event].push(callback); // ➕ 添加监听者
    }
    ...
    emit(event, data) {
        ...
        if (this.listeners[event]) {
            // 遍历所有回调函数并执行，把数据传给它们
            this.listeners[event].forEach(callback => callback(data)); // 📢 逐个通知监听者
        }
    }
    ...
    off(event, callback) {
        ...
        if (!this.listeners[event]) return; // 🛑 如果没人监听，直接返回
        // 过滤掉要移除的那个回调函数
        this.listeners[event] = this.listeners[event].filter(cb => cb !== callback); // 🗑️ 移除指定的回调函数
    }
}
