/* ==========================================================================
   📃 文件功能 : 系统加载器 (System Loader)
   ⚡ 逻辑摘要 : 负责系统的启动流程，包括加载配置、动态导入应用模块、初始化系统服务、清理僵尸数据等。
   💡 易懂解释 : 这是系统的“启动引擎”！它负责把所有零件（应用）组装起来，加上油（配置），然后一脚油门轰鸣启动！🏎️
   🔋 未来扩展 : 支持应用依赖解析，并行加载优化，加载进度条显示。
   📊 当前状态 : 活跃 (更新: 2025-12-03)
   🧱 loader.js 踩坑记录 (累积，勿覆盖) :
      1. [2025-12-03] [已修复] [配置丢失]: prune 逻辑过于激进，误删了动态注册的子窗口配置 -> 修改 validIds 收集逻辑，包含 relatedApps (Line 180)
   ========================================================================== */
...
//
//  ⚙️ 触发源:
//      window.onload -> init() -> setupBusinessLogic()
// =============================================================================
function setupBusinessLogic() {
    ...
    //
    //  🎨 用途:
    //      使用 requestAnimationFrame 驱动的渲染循环，用于更新实时画面。
    //      避免高频网络事件直接操作 DOM 导致卡顿。
    //
    //  💡 易懂解释:
    //      就像放电影一样，一帧一帧地把画面画到屏幕上，而不是来一张贴一张（那样会手忙脚乱）。
    //
    //  ⚠️ 警告:
    //      性能敏感: 此函数每秒可能执行 60 次，必须保持极简。
    //
    //  ⚙️ 触发源:
    //      net:frame 事件 -> requestAnimationFrame -> renderLoop()
    // =============================================================================
    const renderLoop = () => {
...
window.onload = async () => {
    ...
    //
    //  🎨 用途:
    //      当网页所有资源加载完毕后执行。负责初始化 Store、加载应用列表、
    //      动态导入模块、清理数据、启动窗口管理器。
    //
    //  💡 易懂解释:
    ...
    //
    //  ⚠️ 警告:
    //      初始化失败: 如果 JS 报错，可能会导致这里的初始化代码中断执行，整个页面瘫痪。
    //
    //  ⚙️ 触发源:
    //      Browser -> DOMContentLoaded -> window.onload
    // =============================================================================
    ...
    try {
        ...
        // =============================================================================
        //  🎉 检查更新 (serverApp, cachedApp)
        //
        //  🎨 用途:
        //      对比服务器返回的应用元数据和本地缓存的元数据，决定是否需要更新。
        //      优先对比代码行数 (line_count) 以防止版本号未更新的情况。
        //
        //  💡 易懂解释:
        //      看看手里的说明书是不是旧的？如果页数都不一样了，那肯定是改过了，得换新的！
        //
        //  ⚠️ 警告:
        //      无特殊风险。
        //
        //  ⚙️ 触发源:
        //      window.onload -> 遍历 apps -> checkUpdate()
        // =============================================================================
        ...
        // =============================================================================
        //  🎉 动态加载应用 (path, isSystem)
        //
        //  🎨 用途:
        //      使用 dynamic import() 语法异步加载应用模块。
        //      处理缓存破坏 (添加 ?v=timestamp) 和 config 导出检查。
        //
        //  💡 易懂解释:
        //      去仓库把那个叫 path 的零件拿过来，顺便看看说明书 (config) 在不在。
        //
        //  ⚠️ 警告:
        //      网络错误: 如果文件不存在或网络断开，会抛出异常 (已捕获)。
        //
        //  ⚙️ 触发源:
        //      window.onload -> Promise.all -> loadApp()
        // =============================================================================
        const loadApp = async (path, isSystem) => {
            try {
                console.log(`⏳ 正在加载应用: ${path}`); // 📝 加载日志
                // 添加版本号参数以破坏浏览器缓存 (解决"旧JS"问题)
                const url = `${path}?v=${Date.now()}`; 
                const m = await import(url); // 💖 动态导入模块
