// =================================
//  🎉 全局配置文件 (无参数)
//
//  🎨 代码用途：
//     存放全站通用的常量配置，如服务器地址、壁纸列表、默认应用数据等。
//
//  💡 易懂解释：
//     这是系统的“设置中心”。想改服务器地址？想加一张新壁纸？都在这里改。
//
//  ⚠️ 警告：
//     修改此文件后需要刷新页面才能生效。若格式出错（如少个逗号）会导致整个网页无法加载。
// =================================

// =================================
//  🎉 服务器地址配置 (无参数)
//
//  🎨 代码用途：
//     定义后端服务器的 HTTP 和 WebSocket 地址。
//     支持 Web端 (Web_compute_high) 和 Agent端 (Agent_angel_server) 的分离。
//
//  💡 易懂解释：
//     这里填服务器的身份证！🏠
//     SERVER_HOST: 填你的真实服务器IP或域名 (例如 "192.168.1.100" 或 "www.angel-ai.com")
//
//  ⚠️ 警告：
//     不要带 http:// 前缀，只填 IP 或 域名。
// =================================
export const SERVER_HOST = "localhost"; // 🏠 服务器主机地址

// 🔗 Web端高算力节点 (处理登录、存储) - Port 9000 (Rust Axum High)
export const WEB_API_URL = `http://${SERVER_HOST}:9000`; // 💖 Web API 基础地址

// 🔗 Agent端智能体节点 (处理AI、WebSocket) - Port 8000
export const AGENT_API_URL = `http://${SERVER_HOST}:8000`; // 💖 Agent API 基础地址
export const WS_URL = `ws://${SERVER_HOST}:8000/ws`; // 💖 WebSocket 连接地址

// =================================
//  🎉 浏览器配置 (无参数)
//
//  🎨 代码用途：
//     标记系统使用的浏览器通道 (msedge, chrome, 或 null)。
// =================================
export const BROWSER_CHANNEL = null; // 🌐 浏览器通道配置

// =================================
//  🎉 资源路径配置 (无参数)
//
//  🎨 代码用途：
//     定义静态资源的根目录路径。
//
//  💡 易懂解释：
//     告诉系统仓库在哪里。如果以后把 assets 文件夹改名叫 static，只需要改这里这一个地方。
//
//  ⚠️ 警告：
//     末尾必须带斜杠 /。
// =================================
export const ASSETS_PATH = "js/system/assets/"; // 📂 系统级静态资源根目录

// =================================
//  🎉 默认壁纸 (无参数)
//
//  🎨 代码用途：
//     系统初始化时默认使用的壁纸 URL。
//
//  💡 易懂解释：
//     第一次打开系统时看到的那个背景。
//
//  ⚠️ 警告：
//     默认取列表中的第一张。
// =================================
export const DEFAULT_WALLPAPER = `${ASSETS_PATH}wp-1.avif`; // 🖼️ 默认显示的壁纸

// =================================
//  🎉 默认应用列表 (无参数)
//
//  🎨 代码用途：
//     定义客户端本地的默认应用元数据（图标、颜色、名称）。
//     用于在服务器数据缺失（如缺少图标）时进行回退补充。
//
//  💡 易懂解释：
//     这是本地的“备份菜单”。如果服务器只说了“有红烧肉”但没给照片，
//     我们就从这里找一张红烧肉的照片贴上去。📸
// =================================
export const DEFAULT_APPS = {
    "win-angel": { 
        id: "win-angel", 
        filename: "browser.js", 
        version: "1.2.1", 
        icon: "M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z", 
        color: "#6c5ce7", 
        name: "探索之窗" 
    },
    "win-personalization": { 
        id: "win-personalization", 
        filename: "personalization.js", 
        version: "1.0.1", 
        // 🧱 [2025-12-17] 修复: SVG path 中 ".59-.22" 改为 "-.59.22"
        icon: "M19.14 12.94c.04-.3.06-.61.06-.94 0-.32-.02-.64-.07-.94l2.03-1.58c.18-.14.23-.41.12-.61l-1.92-3.32c-.12-.22-.37-.29-.59-.22l-2.39.96c-.5-.38-1.03-.7-1.62-.94l-.36-2.54c-.04-.24-.24-.41-.48-.41h-3.84c-.24 0-.43.17-.47.41l-.36 2.54c-.59.24-1.13.57-1.62.94l-2.39-.96c-.22-.08-.47 0-.59.22L4.09 8.87c-.12.21-.08.47.12.61l2.03 1.58c-.05.3-.09.63-.09.94s.02.64.07.94l-2.03 1.58c-.18.14-.23.41-.12.61l1.92 3.32c.12.22.37.29.59.22l2.39-.96c.5.38 1.03.7 1.62.94l.36 2.54c.05.24.24.41.48.41h3.84c.24 0 .44-.17.47-.41l.36-2.54c.59-.24 1.13-.58 1.62-.94l2.39.96c.22.08.47 0 .59-.22l1.92-3.32c.12-.22.07-.47-.12-.61l-2.01-1.58zM12 15.6c-1.98 0-3.6-1.62-3.6-3.6s1.62-3.6 3.6-3.6 3.6 1.62 3.6 3.6-1.62 3.6-3.6 3.6z", 
        color: "#e17055", 
        name: "个性化" 
    },
    "win-taskmgr": { 
        id: "win-taskmgr", 
        filename: "task_manager.js", 
        version: "1.0.1", 
        icon: "M3 13h2v-2H3v2zm0 4h2v-2H3v2zm0-8h2V7H3v2zm0-8h2V7H3v2zm4 4h14v-2H7v2zm0 4h14v-2H7v2zM7 7v2h14V7H7z", 
        color: "#d63031", 
        name: "活力源泉" 
    },
    "win-manual": { 
        id: "win-manual", 
        filename: "manual.js", 
        version: "1.0.1", 
        icon: "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-6h2v6zm0-8h-2V7h2v2z", 
        color: "#0984e3", 
        name: "光明指引" 
    },
    "win-performance": { 
        id: "win-performance", 
        filename: "performance.js", 
        version: "1.0.1", 
        icon: "M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z", 
        color: "#6c5ce7", 
        name: "性能调优" 
    },
    "win-intelligence": { 
        id: "win-intelligence", 
        filename: "intelligence.js", 
        version: "1.0.1", 
        icon: "M20 2H4c-1.1 0-1.99.9-1.99 2L2 22l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm-7 9h-2V5h2v6zm0 4h-2v-2h2v2z", 
        color: "#00b894", 
        name: "智慧锦囊" 
    }
    // 🧱 [2025-12-17] 移除不存在的应用:
    // - win-filemanager (filemanager.js 不存在)
    // - win-notepad (notepad.js 不存在)
    // - win-settings (settings.js 不存在)
    // - win-terminal (terminal.js 不存在)
};

export const VERSION = '1.0.0'; // 💖 系统核心模块版本号
