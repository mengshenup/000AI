...
// =================================
//  🎉 小天使台词库 (已移至 angel_data.js)
// =================================

// =================================
//  🎉 应用启动语录库 (已移至 angel_data.js)
// =================================
...
export class AngelApp {
    ...
    //
    //  🎨 代码用途：
    //     将小天使的位置、旋转和系统设置恢复到默认状态。
    //
    //  💡 易懂解释：
    //     小天使累了或者迷路了，就用这个魔法把她变回最初的样子！
    //     一切从头开始，焕然一新~ ✨
    //
    //  ⚠️ 警告：
    //     会清除 localStorage 中的相关设置并刷新页面。
    // =================================
    resetState() {
        console.log("执行小天使重置指令...");
        // 1. 重置位置
        if (this.group) {
            this.group.position.set(0, 0, 0); // 📍 重置位置
            this.group.rotation.set(0, 0, 0); // 🔄 重置旋转
        }
        // 2. 重置交互状态
        this.state = { r: false, sx: 0, ir: 0 }; // 🔄 重置交互变量
        // 3. 清除本地存储的静音设置等 (可选)
        localStorage.removeItem('angel_is_muted'); // 🧹 清除静音设置
        localStorage.removeItem('angel_performance_mode'); // 🧹 清除性能设置
        localStorage.removeItem('angel_force_cpu'); // 🧹 清除兼容模式设置
    ...
    //
    //  💡 易懂解释：
    //     这是小天使的“诞生仪式”！搭建舞台（场景）、打光（灯光）、架摄像机，
    //     然后把小天使请出来~ 🎬
    //
    //  ⚠️ 警告：
    ...
    setPerfMode(mode) {
        ...
        this.targetFPS = this.perfMode === 'low' ? 30 : 60; // 🎯 目标帧率
        this.frameInterval = 1000 / this.targetFPS; // ⏱️ 帧间隔
        
        // 如果渲染器已存在，动态调整参数
        if (this.renderer) {
            this.renderer.setPixelRatio(this.perfMode === 'low' ? 1 : window.devicePixelRatio); // 📉 调整像素比
        }
    }
    ...
    //
    //  💡 易懂解释：
    //     演出结束啦！拆掉舞台，收好道具，打扫干净，
    //     不给电脑留下一点垃圾~ 🧹
    //
    //  ⚠️ 警告：
    //     必须彻底清理 renderer 和 scene，否则 WebGL 上下文可能不会释放。
    // =================================
    onDestroy() {
        this.isRunning = false; // 🛑 停止运行
        if (this.renderer) {
            this.renderer.dispose(); // 🗑️ 释放渲染器资源
            this.renderer.forceContextLoss(); // 🗑️ 强制丢失上下文
    ...
    //
    //  💡 易懂解释：
    //     这是小天使的“心跳”！每一帧都让她上下浮动一点点，翅膀扇动一下下，
    //     看起来是活的！💓
    //
    //  ⚠️ 警告：
    ...
    //
    //  🎨 代码用途：
    //     为聊天输入框、发送按钮和语音按钮绑定事件。
    //
    //  💡 易懂解释：
    //     给聊天框通上电！让按钮能按，让回车键能发消息，让麦克风能听见声音。🔌
    //
    //  ⚠️ 警告：
    //     依赖 DOM 元素 #angel-input, #btn-send, #btn-voice。
    // =================================
    bindChatEvents() {
    ...
    //
    //  🎨 代码用途：
    //     切换静音标志位，保存到 localStorage，并更新 UI 和语音状态。
    //
    //  💡 易懂解释：
    //     给小天使戴上口罩（静音）或者摘下来。嘘~ 安静点！🤫
    //
    //  ⚠️ 警告：
    //     静音后，所有 TTS 语音都会被拦截。
    // =================================
    toggleMute() {
        this.isMuted = !this.isMuted; // 🔄 切换状态
        localStorage.setItem('angel_is_muted', this.isMuted); // 💾 保存状态
        this.updateMuteIcon(); // 🔄 更新图标
        
        if (this.isMuted) {
            window.speechSynthesis.cancel(); // 🛑 立即停止发声
        } else {
            this.speak("语音功能已开启"); // 🗣️ 提示语音开启
        }
    }
    ...
    //
    //  🎨 代码用途：
    //     根据当前静音状态显示或隐藏对应的 SVG 图标。
    //
    //  💡 易懂解释：
    //     把喇叭图标换成“有声音”或者“没声音”的样子，让你一眼就能看出来。👀
    //
    //  ⚠️ 警告：
    //     依赖 DOM 元素 #icon-sound-on 和 #icon-sound-off。
    // =================================
    updateMuteIcon() {
        const iconOn = document.getElementById('icon-sound-on'); // 🔊 开启图标
        const iconOff = document.getElementById('icon-sound-off'); // 🔇 关闭图标
    ...
    //
    //  🎨 代码用途：
    //     调用浏览器 SpeechSynthesis API 朗读文本。
    //
    //  💡 易懂解释：
    //     小天使开口说话啦！把文字变成声音，念给你听~ 🗣️
    //
    //  ⚠️ 警告：
    //     如果浏览器不支持 TTS，或者被静音，或者没有中文语音包，可能无法发声。
    // =================================
    speak(text) {
    ...
    //
    //  💡 易懂解释：
    //     把聊天的小窗口弹出来，或者收回去。弹出来的时候还会顺便跟你打个招呼~ 👋
    //
    //  ⚠️ 警告：
    //     依赖 DOM 元素 #angel-chat。
    // =================================
    toggleChat() {
        const chatBox = document.getElementById('angel-chat'); // 💬 聊天框容器
        const input = document.getElementById('angel-input'); // ⌨️ 输入框
        
        if (chatBox) {
            chatBox.classList.toggle('active'); // 🔄 切换激活状态
    ...
    handleSend() {
        ...
        const text = input.value.trim(); // 📝 获取文本
        if (!text) return;
    ...
    //
    //  🎨 代码用途：
    //     调用浏览器 SpeechRecognition API 进行语音转文字。
    //
    //  💡 易懂解释：
    //     把耳朵竖起来！听听你在说什么，然后把声音变成文字写在输入框里。👂
    //
    //  ⚠️ 警告：
    //     仅支持 Chrome/Edge 等 Webkit 内核浏览器，且需要麦克风权限。
    // =================================
